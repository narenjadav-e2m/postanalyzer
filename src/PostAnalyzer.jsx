...
function SettingsModal({ isOpen, onClose, users = [] }) {
  ...
  useEffect(() => {
    if (isOpen) {
      // Load settings from localStorage or WordPress options
      const savedApiKey = localStorage.getItem('postanalyzer_groq_api_key') || '';
      const savedAuthor = localStorage.getItem('postanalyzer_author_validate') || '';
  ...
  const handleSave = async () => {
    ...
    if (!apiKey || apiKey.trim() === '') {
      setSavedMessage('Please enter a Groq API key.');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem('postanalyzer_groq_api_key', apiKey.trim());
        localStorage.setItem('postanalyzer_author_validate', selectedAuthor);
  ...
  const loadSavedSettings = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // API key is masked in response, so check localStorage
        const savedApiKey = localStorage.getItem('postanalyzer_groq_api_key') || '';
        setApiKey(savedApiKey);
      } else {
    ...
    <div className="settings-modal-overlay">
      <div className="settings-modal-container">
        ...
        <div className="settings-form-group">
          <label htmlFor="groq-api-key" className="settings-form-label">
            Groq AI API Key
          </label>
          ...
          {showApiKeyHelp && (
            <div className="settings-help-popover">
              <h4 className="settings-help-title">How to get Groq API Key:</h4>
              <ol className="settings-help-list">
                <li>Visit <a href="https://console.groq.com" target="_blank" rel="noopener noreferrer" className="settings-help-link">console.groq.com</a></li>
        ...
        {/* Save Message */}
        {savedMessage && (
          <div className={`settings-message ${savedMessage.includes('success')
...
function PostAnalyzer() {
  ...
  useEffect(() => {
    ...
    const loadPosts = async () => {
      ...
      try {
        const res = await fetch(postanalyzerWP.restUrl + 'posts?per_page=100', {
          credentials: 'same-origin',
          headers: { 'X-WP-Nonce': postanalyzerWP.nonce }
        });
  ...
  return (
    <div className="postanalyzer-wrapper" ref={fancyboxRootRef}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="head-title">PostAnalyzer</h1>
          <p className="head-text">
            {loadingPosts ? 'Loading available posts...' :
              hasPosts ? 'Choose a post to generate an automated QA & SEO report.' :
        ...
        <>
          <div className="flex items-center gap-3 mb-2 max-w-3xl">
            <select
              ref={selectRef}
              value={postId}
              onChange={(e) => { setPostId(e.target.value); if (error) setError(null); }}
              className="small border p-2 rounded flex-1 !text-base px-4 py-1 hidden max-w-[600px]"
            ...
            <button
              ...
              className="bg-blue-600 hover:not-disabled:bg-blue-500 text-white px-4 py-2 text-base rounded cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed max-w-[200px] min-w-[130px]"
            >
            ...
            <button
              onClick={handleReset}
              className={`${loading ? 'bg-red-600 hover:bg-red-500 text-white' : 'bg-gray-200 hover:bg-gray-300 text-gray-800'} px-4 py-2 text-base rounded cursor-pointer max-w-[100px] min-w-[80px] transition-colors`}
            >
              {loading ? 'Abort' : 'Reset'}
            </button>
          </div>
